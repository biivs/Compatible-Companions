...
def prepare_data(raw_data):
    ...
    tfidf_matrix = tfidf_vectorizer.fit_transform(df['full_description'].fillna(""))

    svd = TruncatedSVD(n_components=100, random_state=42)
    lsa_matrix = svd.fit_transform(tfidf_matrix)

    semantic_model = SentenceTransformer('all-MiniLM-L6-v2')
    semantic_embeddings = semantic_model.encode(df['full_description'].fillna(""), convert_to_tensor=True)

    return df, tfidf_vectorizer, tfidf_matrix, svd, lsa_matrix, semantic_model, semantic_embeddings

animals_df, tfidf_vectorizer, tfidf_matrix, svd, lsa_matrix, semantic_model, semantic_embeddings = prepare_data(raw_data)
del raw_data
gc.collect()
...
def similarity_chart():
    ...
    if not traits_in_query:
        traits_in_query = ['playful', 'calm', 'friendly']

    animal_text = (animal['full_description'] or "").lower()

    def score_trait(trait):
        return 1.0 if re.search(rf"\\b{re.escape(trait)}\\b", animal_text) else 0.1
